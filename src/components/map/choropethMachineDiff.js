...
let zoomedColor = "#90e0ef";

const diffScale = d3.scaleLinear([0, 1], ["#90e0ef", "#03045e"])
    ...
    return new Promise(res => {
        d3.select('#svgmap')
            ...
            .style('opacity', function(d){
                if(this.hasAttribute('data-seat-num')){
                    return getWinner(d, 'declaredVotes').voteDifference == 0 ? 0.4 : 1;
                }else{
                    return 1
                }
            })
            ...
            var panZoom = zoom('#svgmap', {
                ...
                beforePan: function (oldPan, newPan) {
                    ...
                    */
                    ...
                    //return customPan
                    let pw = sizes.viewBox.width * sizes.realZoom + (sizes.width * 0.4);
                    let pictureOffScreenRight = Math.round(pw + newPan.x - sizes.width)
                    let panx = true;
                    if (newPan.x > (sizes.width * 0.4)) {
                        panx = false
                    ...
                    let ph = (sizes.viewBox.height) * sizes.realZoom + (sizes.height * 0.4);;
                    let pictureOffScreenBottom = Math.round(ph + (newPan.y - pTop) - sizes.height)
                    let pany = true;

                    console.log(pTop, pictureOffScreenBottom)

                    if (Math.round(newPan.y) - 10 > Math.round(pTop) + (sizes.height * 0.4)) {
                        pany = false
        ...
        [
            new Promise(res => {
                d3.selectAll('path[data-seat-num]')
                    .transition()
                    .duration(250)
                    .style("opacity", (d) => (filterConstit(d, filterObj, key) 
                        ? getWinner(d, 'declaredVotes').voteDifference == 0 ? 0.4 : 1 
                        : 0.4))
